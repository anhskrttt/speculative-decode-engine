...
# This requests one actual number on the CPU.
value = out[0, 0].item()
value_end = time.perf_counter()

print(f"Launch only: {(launch_end - start) * 1000:.3f} ms")
print(f"Reading one value with .item(): {(value_end - launch_end) * 1000:.3f} ms")
print(f"Total: {(value_end - start) * 1000:.3f} ms")
print("First result value:", value)
